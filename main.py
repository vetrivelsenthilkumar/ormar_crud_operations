...
async def mail(student_id: int):
    students = await models.Student.objects.get(id=student_id)
    await fetch_all_subject_mark(student_id=student_id)
    ...
    with open(f"/home/vetrivel/PycharmProjects/crud_operation/{students.name}.pdf", "rb") as f:
...
async def mail(student_id: int):
    students = await models.Student.objects.get(id=student_id)
    await fetch_data_in_csv_file(student_id=student_id)
    mail_content = '''Student Details'''
    sender_address = 'vetrisenthilmkce@gmail.com'
    sender_pass = 'qxqmgpvvghmopyra'
    receiver_address = 'vetrisenthilmkce@gmail.com'
    ...
    attach.set_payload((attach_file).read())
    encoders.encode_base64(attach)
    with open(f"/home/vetrivel/PycharmProjects/crud_operation/{students.name}.xlsx", "rb") as f:
